...
from core.Handshake import ServerBroadcast, BroadcastListener
...
class TestSockets(unittest.TestCase):
    ...
    def init_tcp_chat_client(self):
        callbacks = (self.ready_for_chat, self.on_send, self.on_receive)
        client_connection = ChatClient('localhost', 'localhost', callbacks)
    ...
    def init_tcp_chat_server(self, callbacks):
        server_connection = ServerConnectionChat('localhost', callbacks)
    ...
    def init_tcp_transport_client(self):
        callbacks = (self.transfer_tcp_send, self.transfer_tcp_receive)
        client_connection = TransferClient('localhost', 'localhost', callbacks)
        client_connection.start()

    def init_tcp_transport_server(self, callbacks):
        server_connection = ServerConnectionTransfer('localhost', callbacks)
        server_connection.start()

    def test_udp_pair_same_channels(self):
        self.udp_pair_helper("puppies", "puppies", False)

    def test_udp_pair_different_channels(self):
        self.udp_pair_helper("puppies", "kittens", True)

    def udp_pair_helper(self, broadcast_channel, listener_channel):
